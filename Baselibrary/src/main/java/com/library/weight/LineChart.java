...
import com.library.utils.CalculateUtil;
...
public class LineChart extends View {
    ...
    private void init(Context context) {
        ...
        mContext = context;
        mLeftMargin = ConvertUtils.dp2px(16);
        mTopMargin = ConvertUtils.dp2px(30);

        mBgPaint = new Paint();
        mBgPaint.setColor(Color.WHITE);

        mAxisPaint = new Paint();
        mAxisPaint.setStrokeWidth(ConvertUtils.dp2px(1));

        mTextPaint = new Paint();
        mTextPaint.setAntiAlias(true);
        mTextPaint.setTextSize(ConvertUtils.dp2px(10));
    ...
    private void drawXAxisText(Canvas canvas) {
        ...
        for(int i = 0;i<mData.size();i++){
            ...
            if((mXStartIndex+distance)>=mXStartIndex&&(mXStartIndex+distance)<(mTotalWidth-mLeftMargin*2)){
                canvas.drawText(text, mXStartIndex+distance-mTextPaint.measureText(text)/2, mPaintBottom + ConvertUtils.dp2px(10), mTextPaint);
            }
        }
    }
    ...
    private void drawCircles(Canvas canvas) {
        for (int i = 0; i < mData.size(); i++) {
            ...
            String text = (mData.get(i).getyValue())+"";
            canvas.drawText(text,mLinePoints.get(i), (mPaintBottom - mData.get(i).getyValue() * mMaxHeight / mMaxDivisionValue)*percent, mTextPaint);
            //只有在可见的范围内才绘制
    ...
    private String getTextValue(int i) {
        String text = "";
        if (mData.size()> 1) {
            text = String.valueOf((int)(0.2*mMaxDivisionValue*i));
        } else if (mData.size() > 0 && mData.size() <= 1) {
