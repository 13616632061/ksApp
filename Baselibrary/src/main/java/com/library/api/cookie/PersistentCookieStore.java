...
public class PersistentCookieStore implements ICookieStore {
    ...
    private static final String COOKIE_PREFS = "sf_saas_cookie";        //cookie使用prefs保存
    private static final String COOKIE_NAME_PREFIX = "sf_saas_cookie_";          //cookie持久化的统一前缀

    private final HashMap<String, ConcurrentHashMap<String, Cookie>> cookies;

    public PersistentCookieStore(Context context) {
        SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE);
        cookies = new HashMap<>();

        //将持久化的cookies缓存到内存中,数据结构为 Map<Url.host, Map<Cookie.name, Cookie>>
        Map<String, ?> prefsMap = SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).getAll();
        for (Map.Entry<String, ?> entry : prefsMap.entrySet()) {
            if ((entry.getValue()) != null && !entry.getKey().startsWith(COOKIE_NAME_PREFIX)) {
                ...
                for (String name : cookieNames) {
                    ...
                    String encodedCookie = SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).getString(COOKIE_NAME_PREFIX + name, null);
    ...
    private void saveCookie(HttpUrl url, Cookie cookie, String name) {
        ...
        //文件缓存
        SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).put(url.host(), TextUtils.join(",", cookies.get(url.host()).keySet()));
        SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).put(COOKIE_NAME_PREFIX + name, encodeCookie(new SerializableHttpCookie(cookie)));
    }
    ...
    public boolean removeCookie(HttpUrl url, Cookie cookie) {
        ...
        if (cookies.containsKey(url.host()) && cookies.get(url.host()).containsKey(name)) {
            ...
            //文件移除
            if (SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).contains(COOKIE_NAME_PREFIX + name)) {
                SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).remove(COOKIE_NAME_PREFIX + name);
            }
            SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).put(url.host(), TextUtils.join(",", cookies.get(url.host()).keySet()));
            return true;
        } else {
    ...
    public boolean removeCookie(HttpUrl url) {
        if (cookies.containsKey(url.host())) {
            ...
            for (String cookieName : cookieNames) {
                if (SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).contains(COOKIE_NAME_PREFIX + cookieName)) {
                    SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).remove(COOKIE_NAME_PREFIX + cookieName);
                }
            }
            SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).remove(url.host());
            //内存移除
    ...
    public boolean removeAllCookie() {
        SPUtils.getInstance(COOKIE_PREFS, Context.MODE_PRIVATE).clear();
        cookies.clear();
        return true;
    }
