...
public class QuickSideBarView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        mMaxHeight = SysUtils.dip2px(context, 25);
...
//        Log.v("yiyou", "height: " + mHeight + ", max height: " + mMaxHeight);
        mWidth = getWidth();
        mItemHeight = mHeight / mLetters.size();
        if (mItemHeight > mMaxHeight) {
            mItemHeight = mMaxHeight;
        }
//        mItemHeight = 100;
    }
...
//        final int newChoose = (int) (y / mHeight * mLetters.size());
        final int newChoose = (int)((y - mPaddingTop) / mItemHeight);
//        Log.v("yiyou", "height: " + mHeight + "-" + mItemHeight + "-" + newChoose + "-" + a);

        if (newChoose < mLetters.size()) {
            switch (action) {
