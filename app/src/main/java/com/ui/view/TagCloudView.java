...
public class TagCloudView extends ViewGroup{
	...
	private static final int DEFAULT_RIGHT_IMAGE = R.drawable.ic_right_grey;
	...
	public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		this.tagList = new ArrayList<TextView>();
			...
			endText.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View view) {
					if (onTagClickListener != null) {
						onTagClickListener.onTagClick(-1, null);
					}
				}
			});
		}
	...
	private int getSingleTotalHeight(int totalWidth, int totalHeight) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (child.getTag() != null && Integer.parseInt(child.getTag().toString()) == TYPE_TEXT_NORMAL) {
			...
			for (int i = 0; i < tags.size(); i++) {
				...
				tagView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (onTagClickListener != null) {
							onTagClickListener.onTagClick(finalI, tagView);
						}
					}
				});
				addView(tagView);
	...
	public ArrayList<TextView> getTagList() {
		return this.tagList;
	}
...
//        requestLayout();
	...
	public interface OnTagClickListener{
		void onTagClick(int position, TextView v);
	}
