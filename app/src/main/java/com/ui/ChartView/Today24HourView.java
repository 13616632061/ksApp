...
//            ,R.mipmap.w18, -1, -1, R.mipmap.w19};
    ...
    //销售量
    public  ArrayList<String>  getdataTodaty_salenum( ArrayList<String> data){
        TEMP=null;
        TEMP=data;
        if(data.size()>0) {
            maxTemp = Double.parseDouble(data.get(data.size()-1))+100;
        }
        minTemp =0;
        ITEM_SIZE=17;
        salenum=true;
    ...
    //销售额
    public  ArrayList<String> getdataTodaty_salemoney( ArrayList<String> data){
        TEMP=null;
        TEMP=data;
        if(data.size()>0) {
            maxTemp = Double.parseDouble(data.get(data.size()-1))+2000;
        }
        minTemp =0;
        ITEM_SIZE=17;
        salemoney=true;
    ...
    //利润
    public  ArrayList<String> getdataTodaty_saleprofit( ArrayList<String> data){
        TEMP=null;
        TEMP=data;
        if(data.size()>0) {
            maxTemp = Double.parseDouble(data.get(data.size()-1))+2000;
...
//            hourItem.res = WEATHER_RES[i];
                ...
                Point point = calculateTempPoint(left, right, Double.parseDouble("0.00"));
                HourItem hourItem = new HourItem();
                hourItem.windyBoxRect = rect;
                hourItem.time = time;
//            hourItem.windy = WINDY[i];
                hourItem.temperature = Double.parseDouble("0.00");
                hourItem.tempPoint = point;
//            hourItem.res = WEATHER_RES[i];
                listItems.add(hourItem);
            }
